...
def load_geography(path):
    ...
    test_questions = []
    test_answers = []
    with open(path + "/geography_database/geography.json", "r") as f:
        jd = json.load(f)
        for myjson in jd:
...
def execute_func(sql):
    def get_connection(db_file: str):
        ...
        return conn
    connection = get_connection('database/geography_database/geography_database.sqlite')
...
def main():
    ...
    with PicardLauncher() if picard_args.launch_picard else nullcontext(None):
        ...
        train_questions, train_answers, test_questions, test_answers = load_geography(backend_args.db_path)

        ans = []

        for i in range(len(test_questions)):
        # for i in range(20):
            prompt_questions, prompt_answers = random_sampling(train_questions, train_answers, 1)
            question = test_questions[i]
            label = test_answers[i]
            input = prompt_func(prompt_questions, prompt_answers, question)
            outputs = pipe(
                        inputs=Text2SQLInput(utterance=question, db_id="geography_database"),
